...
\item Низкоорбитальные группировки → высокоточное позиционирование антенн 
...
\column{0.5\textwidth}
\begin{block}{}
Разработка и реализация программно-аппаратного комплекса для высокоточного 
...
\begin{frame}{Анализ и выбор датчиков}
\centering
\begin{tabular}{lccc}
\toprule
...
% \begin{frame}{Прямое преобразование}
% \begin{equation}
...
\begin{frame}{Алгоритм следящего преобразования}
  ...
  \item $\varepsilon = V sin(\omega t) sin(\phi) \cos(\phi') - V sin(\omega t) cos(\phi) \sin(\phi') = \newline = V \sin(\omega t) \sin(\phi - \phi')  \approx V \sin(\omega t) (\phi - \phi') $ 
...
\begin{figure}[ht]
  ...
  \node[right=of integrator] (output) {$\phi'$};
...
\begin{tikzpicture}
    ...
    \node (phi_update) [right of=check, xshift=6cm][draw, rectangle] {\(\phi = \frac{\phi_\text{т} + 360 \cdot n}{k_\text{т}} \) };
